	...
	if([devices count] > 0) {
		...
		if(!error) {
			...
			for ( AVCaptureDeviceFormat * format in [device formats] ) {
				...
				ofLogVerbose("ofAVFoundationGrabberFast") << " supported dimensions are: " << dimensions.width << " " << dimensions.height;
			}
			
			// Set the new dimensions and format
			if( bestFormat != nullptr && bestW != 0 && bestH != 0 ){
				if( bestW != width || bestH != height ){
					ofLogWarning("ofAVFoundationGrabberFast") << " requested width and height aren't supported. Setting capture size to closest match: " << bestW << " by " << bestH<< endl;
				}
			...
			if( framerate > 0 ){
				...
				for(AVFrameRateRange * range in supportedFrameRates){
					...
					if( (floor(range.minFrameRate) <= framerate && ceil(range.maxFrameRate) >= framerate) ){
						ofLogVerbose("ofAVFoundationGrabberFast") << "found good framerate range, min: " << range.minFrameRate << " max: " << range.maxFrameRate << " for requested fps: " << framerate;
						desiredRange = range;
						numMatch++;
					}
				}
				...
				if( numMatch > 0 ){
					...
					ofLogError("ofAVFoundationGrabberFast") << " could not set framerate to: " << framerate << ". Device supports: ";
					for(AVFrameRateRange * range in supportedFrameRates){
...
//            cout << "TEST" << endl;
			...
			if( grabberPtr != NULL && !grabberPtr->bLock ){
                ...
                if( widthIn != grabberPtr->getWidth() || heightIn != grabberPtr->getHeight() ){
                    ofLogError("ofAVFoundationGrabberFast") << " incoming image dimensions " << widthIn << " by " << heightIn << " don't match. This shouldn't happen! Returning.";
                    return;
                }
...
//            cout << ofToString(grabberPtr->counter.getFps()) << endl;
            ...
            CVReturn err = CVOpenGLTextureCacheCreateTextureFromImage(kCFAllocatorDefault, videoTextureCache, cvFrame, NULL, &internalTexture);
            ...
            unsigned int textureCacheID = CVOpenGLTextureGetName(internalTexture);
...
ofAVFoundationGrabberFast::~ofAVFoundationGrabberFast(){
	ofLog(OF_LOG_VERBOSE, "ofAVFoundationGrabberFast destructor");
	close();
}
		
void ofAVFoundationGrabberFast::clear(){
	if( pixels.size() ){
		pixels.clear();
		pixelsTmp.clear();
	}
...
void ofAVFoundationGrabberFast::setDesiredFrameRate(int capRate){
	fps = capRate;
}
...
bool ofAVFoundationGrabberFast::isInitialized() const{
    return bIsInit;
}
...
const ofPixels & ofAVFoundationGrabberFast::getPixels() const{
	return pixels;
}

bool ofAVFoundationGrabberFast::isFrameNew() const{
	return newFrame;
}

void ofAVFoundationGrabberFast::updatePixelsCB(){
	//TODO: does this need a mutex? or some thread protection? 
	bHavePixelsChanged = true;
}

vector <ofVideoDevice> ofAVFoundationGrabberFast::listDevices() const{
...
void ofAVFoundationGrabberFast::setDeviceID(int deviceID) {
	[grabber setDevice:deviceID];
	device = deviceID;
}

bool ofAVFoundationGrabberFast::setPixelFormat(ofPixelFormat PixelFormat) {
